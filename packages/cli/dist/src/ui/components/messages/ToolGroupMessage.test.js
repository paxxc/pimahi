...
describe('<ToolGroupMessage />', () => {
    ...
    describe('Golden Snapshots', () => {
        ...
        it('renders when not focused', () => {
            const toolCalls = [createToolCall()];
            const { lastFrame, unmount } = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, toolCalls: toolCalls, isFocused: false }));
            expect(lastFrame()).toMatchSnapshot();
            unmount();
        });
        ...
        it('renders header when scrolled', () => {
            const toolCalls = [
                createToolCall({
                    callId: '1',
                    name: 'tool-1',
                    description: 'Description 1. This is a long description that will need to be truncated if the terminal width is small.',
                    resultDisplay: 'line1\nline2\nline3\nline4\nline5',
                }),
                createToolCall({
                    callId: '2',
