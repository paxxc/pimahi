...
export const useGeminiStream = (geminiClient, history, addItem, config, settings, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onEditorClose, onCancelSubmit, setShellInputFocused, terminalWidth, terminalHeight, isShellFocused) => {
    ...
    const [toolCalls, scheduleToolCalls, markToolsAsSubmitted, setToolCallsForDisplay, cancelAllToolCalls,] = useReactToolScheduler(async (completedToolCallsFromScheduler) => {
    ...
    const handleChatCompressionEvent = useCallback((eventValue, userMessageTimestamp) => {
        ...
        return addItem({
            type: 'info',
            text: `IMPORTANT: This conversation exceeded the compress threshold. ` +
    ...
    const pendingHistoryItems = useMemo(() => [pendingHistoryItem, pendingToolCallGroupDisplay].filter((i) => i !== undefined && i !== null), [pendingHistoryItem, pendingToolCallGroupDisplay]);
    useEffect(() => {
