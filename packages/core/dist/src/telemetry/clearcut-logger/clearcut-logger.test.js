...
expect.extend({
    ...
    toHaveGwsExperiments(received, expected_exps) {
        const { isNot } = this;
        const exps = received[0].gws_experiment;
        const pass = exps.length === expected_exps.length &&
            exps.every((value, index) => value === expected_exps[index]);
        return {
            pass,
            message: () => `event ${received} ${isNot ? 'has' : 'does not have'} expected exp ids: ${expected_exps.join(',')}`,
        };
    },
});
