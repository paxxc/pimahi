...
async function initOauthClient(authType, config) {
    ...
    if (authType === AuthType.COMPUTE_ADC) {
        try {
            ...
            throw new Error(`Could not authenticate using metadata server application default credentials. Please select a different authentication method or ensure you are in a properly configured environment. Error: ${getErrorMessage(e)}`);
        }
    }
