...
describe('Task', () => {
    ...
    describe('_schedulerToolCallsUpdate', () => {
        let task;
        let setTaskStateAndPublishUpdateSpy;
        beforeEach(() => {
            const mockConfig = createMockConfig();
            const mockEventBus = {
                publish: vi.fn(),
        ...
        it('should set state to input-required when a tool is awaiting approval and none are executing', () => {
            ...
            expect(setTaskStateAndPublishUpdateSpy).toHaveBeenLastCalledWith('input-required', { kind: 'state-change' }, undefined, undefined, true);
        });
